...
//checknode的路径上加一级寄存器，缓解时序压力（或者两级）
class VariableNodepip extends Module with COMMON {
  val io = IO(new Bundle {
    val LLrin    = Input(UInt(LLRWIDTH.W))
    val Checkin  = Input(Vec(ROWNUM, UInt(C2VWIDTH.W)))
    val Checkout = Output(Vec(ROWNUM,UInt(V2CWIDTH.W)))
  ...
  val min = Wire(UInt(C2VWIDTHCOL.W))
  min := ~(MAXC2V.U(C2VWIDTHCOL.W))+1.U
  val checkimm = VecInit(Seq.fill(ROWNUM)(0.U(V2CWIDTH.W)))
  for(i <- 0 until ROWNUM) {
   checkimm(i) :=Mux( checkoutAPP(i).asSInt <=(-MAXV2C.S),min,Mux(checkoutAPP(i).asSInt>=MAXV2C.S,MAXV2C.U,checkoutAPP(i)(V2CWIDTH-1,0) ))  
  }

  //延迟两拍输出给checknode和out
  val checknext = RegNext(checkimm)
  io.Checkout := RegNext(checknext)
  val appimm = sum + LLrinext
  val appnext = RegNext(appimm)

  io.APPout := RegNext(appnext)
  //GenerateIO.Gen(1)
}
