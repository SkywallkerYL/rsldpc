...
class Decoder2Col0 extends Module with COMMON {
  val io = IO(new Bundle {
    val LLrin     = Input(Vec(BLKSIZE*2,UInt(LLRWIDTH.W)))
...
// EN wire 
  ...
  C2VFlush   := false.B  
  //io.appvalid := false.B
  val localsuccess = Wire(Bool())
  localsuccess:=false.B
  io.Success := RegNext(localsuccess)
  updateen := false.B 
  initialen := false.B
  checkreset := false.B
  updatecheck := false.B
...
// Wire Connect 
  ...
  io.counter := counter 
  val localValid = Wire(Bool())
  localValid := false.B 
  io.OutValid := RegNext(localValid)
  ...
  val localerrorbit = Wire(UInt(11.W))
  localerrorbit := errorcount + localerror
  io.errorbit := RegNext(localerrorbit)
  ...
  switch(currentState){
    ...
    is(decode) {
      ...
      when(counter === 15.U){
        ...
        //when(((rightreg & rightdecide)===1.U)||Iter===1.U) {
          localValid := Mux(io.postvalid,(allcheck1) ===0.U,true.B) 
          localsuccess  := (localerrorbit) ===0.U
          when((Iter===1.U)&&(localerrorbit =/= 0.U)&&(!io.postvalid)){
            errorrecord := 1.U
          }
          //rightreg := 1.U
    ...
    is(postprocess){
      ...
      when(counter === 15.U){
        ...
        //when(((rightreg & rightdecide)===1.U)||Iter===1.U) {
          localValid :=  Mux(postcount=/=maxpostnum.U,(allcheck1) ===0.U,true.B)
          localsuccess := (localerrorbit) ===0.U
          //rightreg := 1.U
          when((postIter===1.U)&&(localerrorbit =/= 0.U)&&(postcount === maxpostnum.U)){
            errorrecord := 1.U
          }
