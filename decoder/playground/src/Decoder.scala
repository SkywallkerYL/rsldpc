...
//32 processing unit 
val Process = Seq.fill(COLNUM)(Module(new ProcessingUnit1))
// 6 check node 
// 按行做， 一次只做一行，  
...
class Decoder1 extends Module with COMMON {
  val io = IO(new Bundle {
    val LLrin     = Input(Vec(COLNUM,UInt(APPWIDTH.W)))
    val Start     = Input(Bool())
    val IterInput = Input(UInt(ITERWITH.W))
...
val Check   = (Module(new CheckNode))
// 32 * 6 MUX 
val AddrMux = Seq.fill(ROWNUM)(Seq.fill(COLNUM)(SyncReadMem(BLKSIZE,UInt(BLKADDR.W))))
//val AddrMux = Seq.fill(ROWNUM)(Seq.fill(COLNUM)(Module(new V2CMux)))
//  6选一 的 addr mux 
val Rowmux  = Seq.fill(COLNUM)(Module(new RowMux))
// EN wire 
...
// Mux ADDR 
    ...
    for(j <- 0 until COLNUM) {
      ...
      for( k <- 0 until BLKSIZE) {
        ...
        for (l <- 0 until BLKSIZE ) {
          val z : Int = rsMatrix(i*BLKSIZE+k)(j*BLKSIZE+l)  
          if(z == 1) {
              AddrMux(i)(j).write(k.U,l.U)  ///.io.SramAddr(k) := l.U
          }
        }
      }
    }
  ...
  for ( i <- 0 until COLNUM) {
    for (j <- 0 until ROWNUM) {
      Rowmux(i).io.V2Caddr(j) := AddrMux(j)(i).read(counter)//.io.ChooseAddr    
    }
    Rowmux(i).io.sel := RowCounter 
  }
  // V2C Sram ADDR  
